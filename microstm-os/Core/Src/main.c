...
int main(void)
{
  ...
  while (1)
  {
	  if (dataRxd == true){
		  dataRxd = false;
		if (strstr(inBuffer,"BLUE ON")) GPIOC->ODR |= (1<<6);
		if (strstr(inBuffer,"BLUE OFF")) GPIOC->ODR &= ~(1<<6);
		  HAL_UART_Transmit(&huart2, (uint8_t*)inBuffer, (uint16_t)strlen(inBuffer), 1000);
		if (strstr(inBuffer, "HELP") || strstr(inBuffer, "help")) {
			const char helpMsg[] =
				"Available commands:\r\n"
